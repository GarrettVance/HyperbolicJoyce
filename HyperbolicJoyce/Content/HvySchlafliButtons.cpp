...
HvyDXBase::HvySchlafliButtons::HvySchlafliButtons(
  ...
  : rb_numberOfRadiettes(p_number_of_radiettes), 
    m_theSelectedButton(0),    //  aka Default Selection is zero;
    m_text(std::wstring(L"NYI")),
    m_panelCornerRadius(17.f),     //  corner radius = 17.f nominal;
    m_panelOutlineThickness(2.f),   //  thickness = 2.f;
    HvyDXBase::HvyWidgets(p_deviceResources, p_pos_horizontal, p_pos_vertical, p_D2D1_ColorF_Enum)
{
...
void HvyDXBase::HvySchlafliButtons::MouseButtonLeft(std::complex<double>   p_mouseClickPhysical)
{
    BOOL containsPointM;
    HRESULT hrM = S_OK;
    ...
    for (uint32_t idx_geometry = 0; idx_geometry < rb_numberOfRadiettes; idx_geometry++)
    {
        ...
        if (containsPointM)
        {
            m_theSelectedButton = idx_geometry;
            break;
        }
    }
}
...
void HvyDXBase::HvySchlafliButtons::Render()
{
    ...
    //  original : context->DrawTextLayout(D2D1::Point2F(0.f, 0.f), m_textLayout.Get(), m_text_SolidColorBrush.Get());
    ...
    for (uint32_t idx_wgt = 0; idx_wgt < rb_numberOfRadiettes; idx_wgt++)
    {
    ...
    for (uint32_t idxDraw = 0; idxDraw < rb_numberOfRadiettes; idxDraw++)
    {
    ...
    float ctr_y = y_centerline + (float)m_theSelectedButton * m_buttonDeltaY;
