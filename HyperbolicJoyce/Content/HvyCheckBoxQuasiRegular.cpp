...
HvyDXBase::HvyCheckBoxQuasiRegular::HvyCheckBoxQuasiRegular(
  ...
  : m_buttonState(false),    //  i.e. the default checkbox state is false; 
    ...
    HvyWidgets(p_deviceResources, p_pos_horizontal, p_pos_vertical, p_D2D1_ColorF_Enum)
{
    ...
    m_ellipseGeometries.assign(1, nullGeometry);
}
...
void HvyDXBase::HvyCheckBoxQuasiRegular::MouseButtonLeft(std::complex<double>   p_mouseClickPhysical)
{
    BOOL containsPointM;
    HRESULT hrM = S_OK;
    ...
    for (uint32_t idx_geometry = 0; idx_geometry < 1; idx_geometry++)  //  TODO : problem ???
    {
        ...
        if (containsPointM)
        {
            //   need either a debouncer, or need Chuck Walbourn's button state tracker: 

            m_buttonState = (m_buttonState == true) ? false : true;
...
void HvyDXBase::HvyCheckBoxQuasiRegular::Render()
{
    ...
    float rb_numberOfRadiettes = 1.0f;
    ...
    OnDemandText(std::wstring(L"Quasi-regular"));
    ...
    for (uint32_t idx_wgt = 0; idx_wgt < 1; idx_wgt++)
    {
    ...
    for (uint32_t idxDraw = 0; idxDraw < 1; idxDraw++)
    {
    ...
    float snb_theSelectedButton = 0.f;  //  fake variable; 
